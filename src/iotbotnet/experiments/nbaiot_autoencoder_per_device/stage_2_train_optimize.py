...
def run_nbaiot_autoencoder_per_device_stage_2(
    config_path: str | Path,
) -> dict[str, Any]:
    """Ejecuta el Stage 2 del autoencoder específico por dispositivo sobre N-BaIoT.

    Para cada dispositivo:
    1. Optimiza hiperparámetros del autoencoder con Optuna/TPE usando únicamente
       train benigno y validación benigna.
    2. Minimiza el percentil configurado de los errores de reconstrucción sobre
       validación benigna.
    3. Reentrena el mejor modelo encontrado sobre train benigno.
    ...
    LOGGER.info("Devices: %d | trials/device: %d", len(devices), n_trials)
    ...
    for device in devices:
        ...
        LOGGER.info("[%s] Best objective value: %.8f", device, best_value)
        ...
        _save_json(device_output_dir / "model_config.json", final_model_config)
        history_df.to_csv(device_output_dir / "history.csv", index=False)
        final_model.save(device_output_dir / "model")
        ...
        _save_scores(train_scores, device_output_dir / "train_reconstruction_errors.parquet")

        LOGGER.info("[%s] Scoring validation split", device)
        val_scores = final_model.score_samples(val_score_dataset)
        _save_scores(val_scores, device_output_dir / "val_reconstruction_errors.parquet")
        ...
        _save_json(device_output_dir / "summary.json", device_summary)
