...
def run_nbaiot_autoencoder_per_device_stage_1(config_path: str | Path) -> dict[str, Any]:
    """Ejecuta el Stage 1 del autoencoder específico por dispositivo sobre N-BaIoT.

    Para cada dispositivo y configuración preliminar:
    1. Entrena un autoencoder únicamente con train benigno.
    2. Monitoriza val_loss sobre validación benigna.
    ...
    global_summary: dict[str, Any] = {
        ...
        "n_trainings": len(devices) * len(model_configs),
        "training": {
            "epochs": epochs,
    ...
    LOGGER.info("Devices: %d | model configs: %d", len(devices), len(model_configs))

    for device in devices:
        LOGGER.info("[%s] Starting device", device)
        ...
        for model_config in model_configs:
            ...
            LOGGER.info("[%s | %s] Starting training", device, config_id)

            run_output_dir = output_dir / "devices" / device / config_id
            ...
            LOGGER.info("[%s | %s] Scoring train split", device, config_id)
            train_scores = model.score_samples(train_score_dataset)
            _save_scores(train_scores, run_output_dir / "train_reconstruction_errors.parquet")

            LOGGER.info("[%s | %s] Scoring validation split", device, config_id)
            val_scores = model.score_samples(val_score_dataset)
            _save_scores(val_scores, run_output_dir / "val_reconstruction_errors.parquet")
            ...
            device_summary["configs"][config_id] = run_summary
            ...
            LOGGER.info("[%s | %s] Finished", device, config_id)

        global_summary["devices"][device] = device_summary
        _save_json(output_dir / "global_summary.json", global_summary)
        LOGGER.info("[%s] Finished device", device)
...
def _save_scores(scores: np.ndarray, output_path: Path) -> None:
    ...
    scores_df = pd.DataFrame({"reconstruction_error": scores.astype(np.float32, copy=False)})
